...
%>	
		...
		%>
<table  width='100%' >
		<tr align="center"><th width='40px'>序号</th><th>专家姓名</th><th>工作单位</th><th>专业领域</th> <th>职称</th><th>联系电话</th><!-- <th width='140px'>操作</th> --></tr >
		<%
			int i=0;
		...
		%>
				...
				<tr align="center" bgcolor="<%=bg %>">
					...
					<td><%=expert.getOrgaName() %></td>
					<td><%=expert.getEngageSpec() %></td>
  				    <td><%=expert.getJobTitle() %></td>
					<td><%=expert.getTel() %></td>
		  <% 
		     if(pass==1){
		   %>
		   ...
		   <% 
		     ...
		     if(pass==2){
		    %>
		    <td><a href="LookExpert.jsp?expertid=<%=expert.getExpertID()%>" id="xslook">查看</a></td>
		   <%	 
		     }
		     if(pass==3){
		    %>
		    ...
		    <% 
		     ...
		     if(pass==4){
		    %>
		    <td><a href="/kjzc/operateexpert?operateid=0&expertid=<%=expert.getExpertID()%>" id="xsdelete">删除|</a><a href="/kjzc/operateexpert?operateid=1&expertid=<%=expert.getExpertID()%>&flag=4" id="xsedit">修改|</a><a href="LookExpert.jsp?expertid=<%=expert.getExpertID()%>" id="xslook">查看</a></td>
		    <% 
		     }
		     if(pass==5)
		     {
