...
<script language="JavaScript" src="qt/js/jquery-1.9.1.min.js"></script>
...
<script type="text/javascript" src="qt/js/qttjfx.js"></script>
  ...
  <table id="__01" width="1046" height="968" border="0" cellpadding="0" cellspacing="0">
    ...
    <tr bgcolor="#FEFEFE">
      ...
      <!-- <table width="259"  border="0" cellpadding="0" cellspacing="0" >
        <tr>
          <td height="41" background="qt/images/temp_09.jpg"><div align="center"><span class="STYLE5">&nbsp;系统介绍</span></div></td>
        </tr>
        <tr>
          <td height="162" background="qt/images/temp_11.jpg">&nbsp;</td>
          </tr>
        <tr>
...
if (pageno>pageCount){
	%>
	<script type="text/javascript">alert("不存在当前页，已返回首页");</script>
	<%
	pageno=1;
}
...
%>
	...
	<div style="background-color: #ffffff; position: relative;margin-top:8px;border: #ffffff 2px solid" >
		...
		<!-- 内容 -->
			...
			<%
			 ...
			 %>
						...
						<div  style="width:420px;  text-overflow:ellipsis;   white-space:nowrap; overflow:hidden;"><a href="qt/zcfxzclook.do?pno=<%=policy.getPno() %>"><%=policy.getPname() %></a></div></td>
			...
			<%}else if(i%2==0){ %>
						...
						<tr >
						<td width="70%" style="margin-left: 30px;"><a href="qt/zcfxzclook.do?pno=<%=policy.getPno() %>"><%=policy.getPname() %></a></td>
							<%if(policy.getApplytime()!=null){ %>
		...
		<%if(pageno>1){ %>
		<a href="qtzhfxfy?pageno=<%if((pageno+1)<1) out.print(1);else out.print(pageno-1); %>">上一页</a>
		 <%} %>&nbsp;&nbsp;
		  <%if(pageno<pageCount){ %>
		 <a href="qtzhfxfy?pageno=<%if((pageno+1)>pageCount) out.print(pageCount);else out.print(pageno+1); %>">下一页</a>
		 <%} %></div>
		<div align="right">
		<%if(pageno>1){ %>
		<a href="qtzhfxfy?pageno=1">首页</a><%} %>&nbsp;&nbsp;
		<%if(pageno<pageCount){ %>
		<a href="qtzhfxfy?pageno=<%=pageCount%>">尾页</a><%} %>
		  跳转到
