<%@ page language="java" contentType="text/html; charset=UTF-8"
	...
	import="com.dqwr.topic.entity.*,com.dqwr.model.Topicmodel, java.util.ArrayList,com.dqwr.dao.*"
	import="com.dqwr.topic.dao.impl.*" pageEncoding="UTF-8"%>
<%
...
<script type="text/javascript">
	function fun(obj) {
	var name = encodeURI($(obj).html());
		$("#find").attr("value", $(obj).html());
		$("#sType0").val("topicname");
		$("#select").click();
		$("#refresh").empty();
		$("#wenzhangis").hide();
		$("#refresh2").show();
	...
	function addRow() {
		...
		e2.id = "sType" + strId;
		e2.size = 1;
		e2.options[0] = new Option("活动名称", "title");
		e2.options[1] = new Option("所属专题", "topicname");
		e2.options[2] = new Option("大致内容", "Content");
		e2.options[4] = new Option("发布时间", "Time");
...
<body>
	<div align="center">
		...
		<table id="__01" width="1046" height="968" border="0" cellpadding="0"
			...
			<tr>
				<td colspan="26" background="images/index_02.gif">
			...
			<tr bgcolor="#FEFEFE">
					...
					<div id="body">
						...
						<div id="body">
							...
							<%
										...
										Connection con1=db1.connection();
										rs1=topicimpl.selecttopictop5(con1);
										while(rs1.next())
										{
										    out.print("<div class='menu'>");
										    out.print("<img style='margin-left: 15px;'src='images/tubiao.jpg' width='12px' height='12px;'>");
											out.print("<a id='"+rs1.getString(2)+"' title='"+rs1.getString(2)+"'onclick='fun(this)'>"+rs1.getString(1)+"</a>");
											out.print("</div>");
										}
							%>
							<!-- 生成动态菜单    结束-->
				...
				<div
						...
						align="center"></div>
					<!--  查询出的具体内容  -->
					<div id="wenzhangis" style="display: none">
						<div id="refresh"></div>
						<div style="height: 300px;"></div>
					</div> <!--  查询出的具体内容  -->
					<div id="refresh2">
						<form name="thisform" action="qtzthd.jsp" method="post"
							onsubmit="return check();">
							<div align="center" class="search"
								style="border-style: groove; border-width: 0px; width: 90%;margin-top:-30px; margin-left: 5%;margin-bottom:14px;">
												...
												<table bgcolor="#FFFFFF" cellspacing="1" cellpadding="0"
													...
													<tbody>
														<tr id="row1">
															...
															<td><select class="sType0" id="sType0" name="sType0"
																size="1">
																	<option value="Title">活动名称</option>
																	<option value="topicname">所属专题</option>
																	<option value="Content">大致内容</option>
																	<option value="Time">发布时间</option>
															</select> <input id="find" type="text" size="50" maxlength="80"
																name="q0" id="q0"> <input id="select"
						...
						<div style="height: 600px; width: 855px; margin-left: 10px"
							...
							<%						//初始化当前页
													...
													ArrayList<TopicNews> topicnewslist;
													int pageCount1;
													//检索模块
													if(num==null){
														System.out.println("你进入无num操作");
														topicnewslist = Topicmodel.topicnewslist1(con, pageno1, pagesize1);
														pageCount1=Topicmodel.topicnewspage1(con, pageno1, pagesize1);
													...
													q0 = java.net.URLEncoder.encode(q0,"utf-8");
													
													//q0=new String(q0.getBytes("ISO8859_1"),"UTF-8");
													...
													//q1=new String(q1.getBytes("ISO8859_1"),"UTF-8");
													...
													//q2=new String(q2.getBytes("ISO8859_1"),"UTF-8");
													...
													//q3=new String(q3.getBytes("ISO8859_1"),"UTF-8");
														}
														topicnewslist = Topicmodel.topicnewslist2(con, pageno1, pagesize1,num,sType0,q0,logic1,sType1,q1,logic2,sType2,q2);
														pageCount1=Topicmodel.topicnewspage2(con, pageno1, pagesize1,num,sType0,q0,logic1,sType1,q1,logic2,sType2,q2);
														if (pageno1>pageCount1){
														    pageno1=1;
														    topicnewslist=Topicmodel.topicnewslist2(con, pageno1, pagesize1,num,sType0,q0,logic1,sType1,q1,logic2,sType2,q2);
															pageCount1=Topicmodel.topicnewspage2(con, pageno1, pagesize1,num,sType0,q0,logic1,sType1,q1,logic2,sType2,q2);
							...
							<table width='100%'>
								<tr align="center" bgcolor="#c0e4fa">
									<th width='40px'>序号</th>
									<th width='600px'>活动标题</th>
									<th>发布时间</th>
								</tr>
													<%
															int i=0;
															for(TopicNews news:topicnewslist){ 
																i=i+1;
																String bg =null;
																if (i%2==1){
																	bg="#ffffff";
								...
								<tr align="left" bgcolor="<%=bg%>">
									...
									<td><a id="href1" href=""
										onclick="createRequest('<%=path%>/qt/lookTopicNews.jsp?topicnewsid=<%=news.getId()%>&type=1','refresh');return false;"><%=news.getTitle()%></a></td>
									<td align="center"><%=news.getTime()%></td>
											<%
													topicnewsid=news.getId();
											%>
								</tr>
								<%
									}else{ bg="#F0F0F0";
			...
			共有<%=i%>条最新记录，当前页第<%=pageno1%>/<%=pageCount1%>页 &nbsp;&nbsp;
           <a href="qtzthd.jsp?pageno=<%if((pageno1+1)<1) out.print(1);else out.print(pageno1-1);%>">上一页</a>&nbsp;&nbsp;<a
			href="qtzthd.jsp?pageno=<%if((pageno1+1)>pageCount1) out.print(pageCount1);else out.print(pageno1+1);%>">下一页</a>
			</div>
		<form name="form1" method="post" action="qtzthd.jsp?num=1&sType0=topicname&q0=<%= q0 %>">
	       <div align="right">
			<a href="qtzthd.jsp?pageno=1">首页</a>&nbsp;&nbsp;<a
			href="qtzthd.jsp?pageno=<%=pageCount1%>">尾页</a> 跳转到 <input
			type="text" size="1" style="width: 20px; height: 15px;"
			name="pageno" id="jumpPageNo" ></input>页&nbsp;&nbsp;<a href="#"	onclick="form1submit()">跳转</a>
			</div>
								</form>
							</div>
