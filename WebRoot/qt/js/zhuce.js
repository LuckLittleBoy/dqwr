...
for(var i=0;i<6&&news.news[i]!=null;i++){
	if(i%2==0){
		strnews += '<tr style="background: #A8D3F3">';
		strnews += '<td width="80%"><a href="qt/qtzclook.do?pno='+news.news[i].zcwh+'">'+news.news[i].zcmc+'</a></td>';
		if(news.news[i].gbrq!=null){
			strnews +=	'<td >['+news.news[i].gbrq+']</td>';
		}
		else{
			strnews +=	'<td >&nbsp;</td>';
		}
		...
		strnews += '<td width="70%" style="margin-left: 30px;"><a href="qt/qtzclook.do?pno='+news.news[i].zcwh+'">'+news.news[i].zcmc+'</a></td>';
		if(news.news[i].gbrq!=null){
			strnews +=	'<td >['+news.news[i].gbrq+']</td>';
		}
		else{
...
function CheckIsValid()
{
	...
	var url="/dqwr/qtlogin?user="+escape(user.value)+"&time="+new Date().getTime()+"&pwd="+escape(pwd.value)+"&randcode="+escape(randcode.value);
	if(user.value==""){
		alert("用户名不能为空");
	}else if(pwd.value==""){
		alert("密码不能为空");	
...
/*function callback()
{
  if(xmlHttp.readyState==4)
  {
    if(xmlHttp.status==200)
    {
      ...
      if(requestmsg=="true"){
    	  window.location.href="dqwr/qt/index.jsp";
    	  alert("登陆成功");
      }
      else{
    	  alert(requestmsg);
