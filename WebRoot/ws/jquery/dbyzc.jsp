...
<%
	...
	System.out.println("带贬义");
	YH user = (YH) request.getSession().getAttribute("user");
	String id = request.getParameter("id");
	Connection con = database.connection();
	ArrayList<ZC>  policytaglist = Brose.dbyPolicy(con, pageno, pagesize,user.getYHID());
	int pageCount=Brose.dbypagecount(con,pageno,pagesize,user.getYHID());
	if (pageno>pageCount){
%>
		
		<%
			pageno=1;
				pageCount=Brose.dbypagecount(con,pageno,pagesize,user.getYHID());
				policytaglist = Brose.dbyPolicy(con, pageno, pagesize,user.getYHID());
			}
			con.close();
		%>
	<table  width='100%' >
	...
	<div style="background: #C0E4FA;position:absolute;" align="left"> 共有 <%=i %> 条最新记录，当前页第<%=pageno %>/<%=pageCount %>页 &nbsp;&nbsp; <a href="dbyzc.jsp?pageno=<%if((pageno+1)<1) out.print(1);else out.print(pageno-1); %>">上一页</a>&nbsp;&nbsp;<a href="dbyzc.jsp?pageno=<%if((pageno+1)>pageCount) out.print(pageCount);else out.print(pageno+1); %>">下一页</a></div>
	<form name="form1" method="post" action="dbyzc.jsp"><div align="right"><a href="dbyzc.jsp?pageno=1">首页</a>&nbsp;&nbsp;<a href="dbyzc.jsp?pageno=<%=pageCount %>">尾页</a>  跳转到
