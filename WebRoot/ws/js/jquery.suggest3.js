		...
		$.suggest3 = function(input, options) {
			...
			function displayItems(items) {
				...
				if (items=='') {//热门城市遍历
					...
					html='<div class="gray ac_result_tip">请输入中文或者↑↓选择</div><ul style="overflow-y:scroll">'+html+'</ul>';
				}
				else {
					/*if (!items)
					return;
			...
			if ($currentResult) {
					...
					if( $(options.dataContainer) ) {
						var olddata = $(options.dataContainer).val()
						if(olddata!=""){	
							olddata=olddata+";";
							ch = new Array;
							ch = olddata.split(";");
							...
							for(i=0;i<ch.length;i++){
								...
								if(ch[i]==$currentResult.attr('rel')){
									j=1;
								}
							}
							if(j==0){
								$(options.dataContainer).val(olddata+$currentResult.attr('rel'));
							}
						}
						else{
							$(options.dataContainer).val(olddata+$currentResult.attr('rel'));
