...
$(document).ready(function () {
	...
	$(function(){
		...
		$("#sxfw").focus(function(){
			var ss;
			for(var i =0 ;sxfw.sxfw[i]!=null;i++){
		...
		$("#zcmc").blur(function(){
			...
			jQuery(".form").html("");
			if(jQuery("#zcmc").val()!=""){
				jQuery.post("../CheckZCMC",{"zcmc":jQuery("#zcmc").val()},function(response){
		...
		$("#zcwh").blur(function(){
			...
			jQuery(".form").html("");
			if(jQuery("#zcwh").val()!=""){
				jQuery.post("../CheckZCWH",{"zcwh":jQuery("#zcwh").val()},function(response){
					jQuery(".form").html(response);
				});
			}
			if(jQuery(".form").html()!=""){
...
$(function(){
	...
	$("#tzly").suggest3(tzlys,{hot_list:tzlys,dataContainer:'#tzlyhidden',attachObject:"#suggest7"});
...
/**
 * 校验所有输入域是否含有特殊符号 所要过滤的符号写入正则表达式中，注意，一些符号要用'\'转义. 试例：
 * if(checkAllTextValid(document.forms[0])) alert("表单中所有文本框通过校验！");
 */
function checkAllTextValid(form) {
	if(jQuery("#content1").text()==""||jQuery("#content1").text()==null){
		alert(jQuery("#content1").val());
		alert("政策内容不能为空");
		return false;
	}
