...
<div style="position: relative;min-height: 490px; " >
    ...
    <br/>
   <form id="example" name="example" method="post" action="/dqwr/editinfo?infoid=<%=info.getInfoId() %>&pagesize=<%=pagesize %>&pageno=<%=pageno %>"target="manFrame">
	<br>
	...
	主题：&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" name="title" style="width:640px;" height="30px" value=<%=info.getTitle() %> ></br>
	发布时间：<input  id="publictime" name="publictime" type="text" onclick="new Calendar().show(this);" value=<%=info.getAddDate()%>>
	<br>
	<%
	 if(info.getType()==1){
		 if(info.getScope()==0){
	...
	%>
	   ...
	   <tr>
		...
		<input type="submit" value="修改" align="center">
	</form>
	</div>
</div>
</div>
