		...
		if(xhr.readyState==4){
			...
			{				
				...
				if(keys.keys=="fail"){
					...
					"<TR align='CENTER'><Th colspan='3'>所有关键字</Th></TR>"+
					"<tr ><td >编号</td><td >关键词名称</td><td >操作</td>";
						for(var i=0;i<10;i++){
							...
							"<td style='padding: 2px;'><button class='btn btn-signup' onclick=alterkey('"+allkeys.keys[i].id+"') >修改</button><button class='btn btn-login'  onclick='delkey("+allkeys.keys[i].id+")' >删除</button></td>"+
						"</tr>";
...
function changemsg(zimu){
	...
	"<tr><th>编号</th><th>关键词名称</th><th>操作</th></tr>";
	if(zimu=="all"){
		currentPage=1;
	//开始插入页面数据
	var msgstr="<table width='80%' class='gridtable'>"+
