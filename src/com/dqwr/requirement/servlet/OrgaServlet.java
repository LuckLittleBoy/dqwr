...
public class OrgaServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String orgareturn=orgaimpl.insertOrga(con, orga);
		String requreturn=requireimpl.insertRequire(con, req);
		PrintWriter out=response.getWriter();
		response.setContentType("text/html;charset=gb2312");
		if(orgareturn.equals("success")&&requreturn.equals("success")){
			out.print("<script language=javascript>" + "if(confirm('填报成功，是否继续填报？')){window.location.href='qt/Requirement.jsp';}else{ window.location.href='ws/xdsh.jsp?reqpass=1';};"+"</script>");
		}else{
