	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			String path = request.getSession().getServletContext()
					.getRealPath("\\attached\\file\\document");
			...
			for (FileItem item : list) {
				...
				if (name.indexOf(".doc") > 0 || name.indexOf(".docx") > 0
						|| name.indexOf(".pdf") > 0|| name.indexOf(".wps") > 0) {
					// 获取表单的属性名字
					...
					// 对传入的非 简单的字符串进行处理 ，比如说二进制的 图片，电影这些
						...
						// 为读取pdf类提供文件名
						request.getSession().setAttribute("uploadfilename",
								filename);
						...
						OutputStream out = new FileOutputStream(new File(path,
								filename));
						response.getWriter()
								.print(" <script language='javascript' type='text/javascript'>alert('上传成功！'); window.location.href='ws/dbyzc.jsp';</script>");
					...
					response.getWriter()
							.print(" <script language='javascript' type='text/javascript'>alert('上传失败！此处只能上传PDF,DOC,DOCX,WPS格式'); window.location.href='ws/dbyzc.jsp';</script>");
				}
			}
