...
public class OperateRequireb extends HttpServlet {
	private static final long serialVersionUID = 1L;
	 DataBaseDao database = null;   
    /**
     * @see HttpServlet#HttpServlet()
     */
    public OperateRequireb() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Orga orga=new Orga();
		if(operateid==0){
			output.print("<script language=javascript>" + "if(confirm('确认删除？')){window.location.href='/dqwr/DeleteRequ?requid="+requid+"&orgaid="+orgaid+"&pageno="+pageno+"';}else{ window.location.href='ws/xdsh.jsp?reqpass=2&pageno="+pageno+"';};"+"</script>");
		}else if(operateid==1){
			...
			request.getRequestDispatcher("qt/editRequire.jsp").forward(request,response);
		}else if(operateid==2){
			request.getRequestDispatcher("qt/RequireBack.jsp").forward(request,response);
		}
