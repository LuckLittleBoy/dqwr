...
public class EditExpertServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String result=expertimpl.updateExpert(con, expert,expertid);
		database.closeConnection(con);
		PrintWriter out=response.getWriter();
		if(result.equals("fail")){
			if(flag.equals("2")){
				out.print("<script language=javascript>alert('修改失败！');document.location.href='ws/expert1.jsp?exppass=2&pageno="+pageno+"';</script>");		
			}else{
				out.print("<script language=javascript>alert('修改失败！');document.location.href='ws/expert1.jsp?exppass=4&pageno="+pageno+"';</script>");	
			}
			
		}else{
			if(flag.equals("2")){
				out.print("<script language=javascript>alert('修改成功！');document.location.href='ws/expert1.jsp?exppass=2&pageno="+pageno+"';</script>");		
			}else{
				out.print("<script language=javascript>alert('修改成功！');document.location.href='ws/expert1.jsp?exppass=4&pageno="+pageno+"';</script>");	
			}
		}
		}
}
